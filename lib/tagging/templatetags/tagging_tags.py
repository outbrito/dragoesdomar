...
def tags_for_articleblog(context):
    from blogdaverdade.models import ArticleBlog
    context['tags_articleblog'] = Tag.objects.usage_for_model(ArticleBlog)[:50]
    return ''

@register.simple_tag(takes_context=True)
def tags_for_retratoblog(context):
    from blogdaverdade.models import RetratoArticleBlog as ArticleBlog
    context['tags_articleblog'] = Tag.objects.usage_for_model(ArticleBlog)[:50]
    return ''